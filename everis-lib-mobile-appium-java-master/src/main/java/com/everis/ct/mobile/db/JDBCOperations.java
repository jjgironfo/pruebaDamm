package com.everis.ct.mobile.db;
...
public class JDBCOperations {
    ...
    private void queryLog(String query) {
        UtilMobile.logger(this.getClass()).log(Level.INFO, "Query >>> {0}", query);
    }
    ...
    public boolean insertNewRow(String schemaTable, String insertColumns, String insertValues) {
        ...
        if (rows > 0) {
            UtilMobile.logger(this.getClass()).info("Una nueva fila fue insertada.");
            inserted = true;
        } else {
            UtilMobile.logger(this.getClass()).info("Ocurrió un error al insertar una nueva fila.");
        }
        return inserted;
    }
    ...
    public boolean updateValue(String schemaTable, String valuesToUpdate, String condition) {
        ...
        int rows = jdbcTemplate.update(query);
        if (rows > 0) {
            UtilMobile.logger(this.getClass()).info("Una nueva fila fue actualizada.");
            updated = true;
        } else {
            UtilMobile.logger(this.getClass()).info("Ocurrió un error al actualizar una fila.");
        }
        return updated;
    }
    ...
    public boolean deleteData(String schemaTable, String condition) {
        ...
        int rows = jdbcTemplate.update(query);
        if (rows > 0) {
            UtilMobile.logger(this.getClass()).info("Una nueva fila fue eliminada.");
            deleted = true;
        } else {
            UtilMobile.logger(this.getClass()).info("Ocurrió un error al eliminar una fila.");
        }
        return deleted;
    }
    ...
    public boolean isTableExist(String catalog, String schemaPattern, String table, String[] types) {
        ...
        try {
            ResultSet tables = Objects.requireNonNull(
